	...
	for _, tt := range successTestCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(actual, tt.expected) {
				t.Fatalf("Build for test case %q returned %s (%s) but was expected to return %s (%s).",
					tt.name, actual, reflect.TypeOf(actual).String(), tt.expected, reflect.TypeOf(tt.expected).String())
			}
		})
	}
}
