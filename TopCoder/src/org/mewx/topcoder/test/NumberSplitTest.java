...
import org.mewx.topcoder.problems.NumberSplit;
...
public class NumberSplitTest {
    ...
    public void longestSequence() throws Exception {
        List<ParsedResultMeta> parsedResultMeta = TestUtils.parseTestData(TestUtils.getFileContentById(4451));
        for (int i = 0; i < parsedResultMeta.size(); i ++) {
            assertEquals(TestUtils.getFailureMessage(i, parsedResultMeta.size(), parsedResultMeta.get(i).getTestArgs()),
                    ...
                    new NumberSplit().longestSequence(BuiltinParser.parseToInt(parsedResultMeta.get(i).getTestArgs())));
        }
