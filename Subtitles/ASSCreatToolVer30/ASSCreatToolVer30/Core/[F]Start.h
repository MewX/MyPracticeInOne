...
#include "..\Test\ComputeTest.h"


static void Start(ScriptInfo TempScriptInfo, short PositionMode)
{
	ass_temp<<">>已进入Core程序"<<endl;

	Width=TempScriptInfo.PlayResX;
	Height=TempScriptInfo.PlayResY;
	...
	ass_temp<<">>已进入Ohead()"<<endl;
	Ohead(TempScriptInfo);
	ass_temp<<"  >>已返回主程序"<<endl;
	ass_temp<<">>已退出Core程序"<<endl;
	...
	if(ofJPrm==true)
	{
		ass_originJPrm.close();//
	}
	if(ofROCK==true)
	{
		ass_originROCK.close();//
	}
	ass_in.close();//
	//关闭文件
}